...
class LedgerApp:
    ...
    def get_public_key(self) -> str:
        ...
        apdu = [self.BNC_CLA, self.BNC_INS_PUBLIC_KEY_SECP256K1,
                0x00, 0x00, len(dongle_path)]
    ...
    def show_address(self):
        ...
        """
        dongle_path = self._parse_hrp(
            self._hrp) + self._parse_hd_path(self._path)
        apdu = [self.BNC_CLA, self.BNC_INS_SHOW_ADDR_SECP256K1,
                0x00, 0x00, len(dongle_path)]
        apdu.extend(dongle_path)
        self._exchange(apdu)
    ...
    def get_address(self) -> dict:
        ...
        dongle_path = self._parse_hrp(
            self._hrp) + self._parse_hd_path(self._path)
        apdu = [self.BNC_CLA, self.BNC_INS_GET_ADDR_SECP256K1,
                0x00, 0x00, len(dongle_path)]
        apdu.extend(dongle_path)
        response = self._exchange(apdu)
        return {
    ...
    def _get_sign_chunks(self, msg: bytes):
        ...
        chunks += [msg[i:i + self.CHUNK_SIZE]
    ...
    def sign(self, msg: bytes) -> str:
        ...
        for idx, chunk in enumerate(chunks):
            apdu = [self.BNC_CLA, self.BNC_INS_SIGN_SECP256K1,
